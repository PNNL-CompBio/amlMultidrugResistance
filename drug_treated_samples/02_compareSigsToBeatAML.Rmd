...
drug_treatments<-c('Venetoclax','Quizartinib - Venetoclax','Azacytidine - Quizartinib','Gilteritinib','Azacytidine - Venetoclax','Azacytidine')
#drug_treatments = c('Venetoclax','Azacytidine - Venetoclax','Azacytidine')
...
calc_sig_in_patients<-function(sensitive_markers,resistant_markers, drug_sig,signame){
 ...
 # print(paste("Found",nrow(drug_vals),'auc vals for',drug_sig))
  ...
  try(pvals<-mdata|>
        ...
        group_by(`Signature type`)|>
        summarize(t.pval=t.test(signature~group)$p.value,
                  w.pval=wilcox.test(signature~group)$p.value))
  
  try(mpvals<-mdata|>
        subset(group%in%c('Middle','Resistant'))|>
        group_by(`Signature type`)|>
        summarize(t.pval=t.test(signature~group)$p.value,
                  w.pval=wilcox.test(signature~group)$p.value))
  
  if(drug_sig=='Azacytidine')
    pvals<-mpvals
  ...
  p1=ggplot(mdata,aes(x=AUC,y=signature,col=`Signature type`))+geom_point()+
    geom_smooth(method=lm, aes(fill=`Signature type`))+
    scale_fill_manual(values=sigcolors)+
    ggtitle(paste(signame,'treated signature vs. patient AUC of ',drug_sig,'\n Rho =',corval))+
    scale_color_manual(values=sigcolors)+theme_bw()
...
pdf(paste0('corStatsLFC',lfc,'.pdf'))

#singles<-c("V-UT","D-UT",'DV-UT')
singles <- c("V-UT",'G-UT','D-UT','GV-UT','DV-UT','GD-UT','GVD-UT')
##let's collect the stats in a table                
res<-do.call(rbind,lapply(singles,function(sn){
    markers=getSigsFromData(condition=sn,protList=rownames(exprs(m_210)),byPval=TRUE,lfcthresh = lfc)
    do.call(rbind,lapply(drug_treatments,function(dr,markers){
        ...
        res|>mutate(Signal=treatlist[[sn]],Drug=dr,lfc=lfc)
  },markers))
}))
...
We now have summary stats we can plot
...
ggsave(paste0("allCorsLFC",lfc,".pdf"))
...
ggsave(paste0("sigCorsLFC",lfc,".pdf"))
...
fulldat<-res|>#rbind(res,res4)|>
  mutate(concordant=ifelse(Rho>0,TRUE,FALSE))|>
  mutate(significant=ifelse(Rho.p<0.05,TRUE,FALSE))

#fulldat$Signal<-factor(fulldat$Signal,levels=c("Venetoclax","Decitabine","Gilteritinib+Decitabine","Decitabine+Venetoclax"))
fulldat$Signal<-factor(fulldat$Signal,levels=c("Venetoclax","Gilteritinib","Decitabine",
                                               "Gilteritinib+Venetoclax","Decitabine+Venetoclax",
                                               "Gilteritinib+Decitabine","Gilteritinib+Decitabine+Venetoclax"))

#fulldat$Drug<-factor(fulldat$Drug,levels=c("Venetoclax","Azacytidine","Azacytidine - Venetoclax"))
fulldat$Drug<-factor(fulldat$Drug,levels=c("Venetoclax", "Gilteritinib","Azacytidine","Quizartinib - Venetoclax","Azacytidine - Venetoclax",  "Azacytidine - Quizartinib"))
