...
calc_sig_in_celllines<-function(sensitive_markers,resistant_markers, drug_sig,signame){
  ...
  mdata$Treatment<-unlist(lapply(mdata$Treatment,function(x) ifelse(x=='none','none',druglist[[x]])))
  ...
  meanSig=mdata|>group_by(Treatment,`Signature type`)|>
    ...
    dplyr::rename(treated=druglist[[drug_sig]])|>
    mutate(difference=treated-none)
  

  ret<-pvals|>
    left_join(meanSig)|>
    mutate(sig=signame,cellLine=druglist[[drug_sig]])
  return(ret)
}
...
res<-do.call(rbind,lapply(sig_names,function(sn){
    markers=getSigsFromData(condition=sn,protList=unique(global$Gene))
    do.call(rbind,lapply(cell_lines,function(dr,markers){
        ...
        res|>mutate(Signature=treatlist[[sn]],CellLine=dr)
  },markers))
}))
...
res|>ggplot(aes(x=cellLine,y=t.pval,fill=Signature))+geom_bar(stat='identity',position='dodge')+facet_grid(`Signature type`~.)+coord_flip()+ggtitle("T-test p-value")+scale_fill_manual(values=dcolors)

res|>subset(t.pval<0.05)|>
  ggplot(aes(x=cellLine,y=difference,fill=Signature))+geom_bar(stat='identity',position='dodge')+facet_grid(`Signature type`~.)+coord_flip()+ggtitle("Significant signals")+scale_fill_manual(values=dcolors)


```

Now we can evaluate how well the signals align

```{r concordance,message=FALSE}
fulldat<-res|>
  mutate(concordant=ifelse(difference>0,TRUE,FALSE))|>
  mutate(significant=ifelse(t.pval<0.05,TRUE,FALSE))

fulldat$Signature<-factor(fulldat$Signature,levels=c("Venetoclax","Gilteritinib","Decitabine","Gilteritinib+Venetoclax","Gilteritinib+Decitabine","Decitabine+Venetoclax","Gilteritinib+Decitabine+Venetoclax"))

fulldat$cellLine<-factor(fulldat$cellLine,levels=c('Gilteritinib','Decitabine',"Gilteritinib+Venetoclax","Gilteritinib+Decitabine","Decitabine+Venetoclax","Gilteritinib+Decitabine+Venetoclax"))
...
subset(fulldat,`Signature type`=='Ratio')|>
  ggplot(aes(y=Signature,x=cellLine,size=abs(difference),col=concordant,shape=significant))+geom_point()+scale_x_discrete(guide = guide_axis(angle = 90))+ggtitle("Ratio signal correlation with Cell line changes")+scale_color_manual(values=tfcolors)

ggsave('AUCCellLineconcordance.pdf')
