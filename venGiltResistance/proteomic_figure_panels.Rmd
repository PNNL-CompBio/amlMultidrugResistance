...
colors=c(`Early resistant`='#EDAE49',Parental='#00798C',`Late resistant`='#D1495B')
...
synStore(File('pcaglobal.pdf',parentId='syn66504211'))
gu|>  
  as.data.frame()|>
  tibble::rownames_to_column('Sample')|>
right_join(meta)|>
...
synStore(File('umapGlobal.pdf',parentId='syn66504211'))
...
synStore(File("pcaphospho.pdf",parentId='syn66504211'))
...
synStore(File('umapPhospho.pdf',parentId='syn66504211'))
...
readr::write_csv(allde,'DiffExFeaturesAllComparisons.csv')
synStore(File('DiffExFeaturesAllComparisons.csv',parentId='syn66504212'))
...
synStore(File('diffExStats_p05_logfc0.pdf',parentId='syn66504212'))
fig
```
...
This figure shows the enrichment analysis using GSEA, GO ORA, Reactome, Reactome ORA, and KSEA. These four analyses quantify, using different methods, how many pathways are enriched in each condition. We use WebGestalt to analyze all the data. 
...
enrichlist<-c("Early resistant_Gilteritinib+Venetoclax_vs_All_Parental",
              "Late resistant_Gilteritinib+Venetoclax_vs_All_Parental",
              "Early resistant_Gilteritinib_vs_All_Parental",
              "Late resistant_Gilteritinib_vs_All_Parental",
              "All_Gilteritinib+Venetoclax_vs_All_Parental",
              "All_Gilteritinib+Venetoclax_vs_All_Gilteritinib",
              'All_Gilteritinib_vs_All_Parental')
...
synStore(File('enrichedGO_fdr05.pdf',parentId='syn66504213'))
fig
...
synStore(File('enrichedGO_ORA_fdr01.pdf',parentId='syn66504213'))
fig
}
...
all_global_Reactome<-do.call(rbind,lapply(enrichlist,function(x){
...
#        tibble::rownames_to_column('Feature')|>
  ...
  print(x)
  res<-NULL
  try(res<-WebGestaltR(enrichMethod='GSEA',enrichDatabase='pathway_Reactome',fdrThr=1.0,
              interestGene=gprots,minNum=3,organism='hsapiens',interestGeneType='genesymbol',isOutput=FALSE)|>
    mutate(condition=x))
  res
}))
...
fig<-ggplot(sigPaths,aes(x=`Treated state`,y=sigTerms,fill=Treatment))+
  geom_bar(stat='identity',position='dodge')+
  coord_flip()+scale_fill_manual(values=dcolors)+
  ggtitle('GSEA Enriched Reactome Pathways')

ggsave('enrichedReactome_fdr05.pdf',fig)
synStore(File('enrichedReactome_fdr05.pdf',parentId='syn66504213'))
fig


##now do Reactome ORA
all_Reactome_ora<-do.call(rbind,lapply(enrichlist,function(x){
...
#  print(length(enriched))
  ...
  try(up<-WebGestaltR(enrichMethod='ORA',minNum=3,
                      enrichDatabase='pathway_Reactome',fdrThr=1.0,
...
sigReactomeORA<-all_Reactome_ora|>
  subset(FDR<0.1)|>
  group_by(condition,direction)|>
  summarize(sigTerms=n_distinct(geneSet))|>
  tidyr::separate(condition,into=c('Treatment','Control'),sep='_vs_')|>
...
fig<-ggplot(sigReactomeORA,aes(x=`Treated state`,y=sigTerms,fill=Treatment))+
  geom_bar(stat='identity',position='dodge')+
  coord_flip()+scale_fill_manual(values=dcolors)+
  facet_grid(direction~.)+
  ggtitle('ORA Enriched Reactome ORA Terms')

ggsave('enrichedReactome_ORA_fdr1.pdf',fig)
synStore(File('enrichedReactome_ORA_fdr1.pdf',parentId='syn66504213'))
fig
...
synStore(File('enrichedKinase_fdr05.pdf',parentId='syn66504213'))
fig
...
#  mutate(description=factor(description,levels=description))|>
  ...
  geom_point()+coord_flip()+scale_color_gradient2(low='darkblue',mid='grey',high='firebrick4')+ theme(axis.text.x=element_text(angle=90, hjust=1))+facet_grid(~Treatment)
#(stat='identity',position='dodge')

readr::write_csv(all_global_enrich,'go_gsea.csv')
synStore(File('go_gsea.csv',parentId='syn66504213'))
ggsave('gvGoTerms_01.pdf',width=7,height=4)
synStore(File('gvGoTerms_01.pdf',parentId='syn66504213'))
```
...
#  subset(`Treated state`=='All')|>
  ...
  geom_point()+coord_flip()+scale_color_gradient2(low='darkblue',mid='grey',high='firebrick4')+
  ggtitle("GO BP ORA Terms")+ theme(axis.text.x=element_text(angle=90, hjust=1))+
  facet_grid(~Treatment)
ggsave('gvORAGoTerms_FDR01.pdf',width=10,height=9)
synStore(File('gvORAGoTerms_FDR01.pdf',parentId='syn66504213'))
...
sigPaths<-all_global_Reactome|>subset(FDR<0.05)|>
  group_by(description)|>summarize(n_distinct(condition))

Reactome_plot<-all_global_Reactome|>
  subset(FDR<0.05)|>
...
ggplot(Reactome_plot,aes(x=description,y=`Treated state`,
                   size=log10p,col=normalizedEnrichmentScore))+
  geom_point()+coord_flip()+scale_color_gradient2(low='darkblue',mid='grey',high='firebrick4')+
  ggtitle("Reactome GSEA Terms")+ theme(axis.text.x=element_text(angle=90, hjust=1))+
  facet_grid(~Treatment)

ggsave('gvReactomePaths_FDR05.pdf',width=15)
synStore(File('gvReactomePaths_FDR05.pdf',parentId='syn66504213'))
readr::write_csv(all_global_Reactome,file='Reactome_GSEA.csv')
synStore(File("Reactome_GSEA.csv",parentId='syn66504213'))
...
ggplot(kegg_plot,aes(x=description,y=`Treated state`,
                   size=log10p,col=normalizedEnrichmentScore))+
  geom_point()+coord_flip()+scale_color_gradient2(low='darkblue',mid='grey',high='firebrick4')+
  ggtitle("Kegg GSEA Terms")+ theme(axis.text.x=element_text(angle=90, hjust=1))+
  facet_grid(~Treatment)

ggsave('gvKeggPaths_FDR05.pdf',width=10,height=8)
synStore(File('gvKeggPaths_FDR05.pdf',parentId='syn66504213'))
readr::write_csv(all_global_kegg,file='kegg_GSEA.csv')
synStore(File("Kegg_GSEA.csv",parentId='syn66504213'))
```
...
kin_counts<-subset(all_kinase_enrich,FDR<0.05)|>
...
ggplot(to_plot,aes(x=Kinase,y=`Treated state`,
                   col=z.score,size=log10p))+  geom_point()+coord_flip()+scale_color_gradient2(low='darkblue',mid='grey',high='firebrick4')+ theme(axis.text.x=element_text(angle=90, hjust=1))+
  facet_grid(~Treatment)
ggsave('sigKinaseSummary.pdf',width=3)

apply(all_kinase_enrich,2,unlist)|>
  as.data.frame()|>
  readr::write_csv(file='kinase_enrichment.csv')
synStore(File('kinase_enrichment.csv',parentId='syn66504213'))
```
...
ggplot(new_subs,aes(x=Drug,y=value,fill=State))+geom_boxplot()+scale_fill_manual(values=colors)+ggtitle("MTOR targets")
 ...
 synStore(File("mtor_targ_boxplot.pdf",parentId='syn66504213'))
...
ggsave('mtor_targ_ridge.pdf',width=8,height=5)
synStore(File('mtor_targ_ridge.pdf',parentId='syn66504213'))
...
new_subs=mutate(pars,Drug='Gilteritinib')|>
 ...
 ggplot(new_subs,aes(x=value,y=SiteID,fill=State,alpha=0.5))+geom_density_ridges()+scale_fill_manual(values=colors)+facet_grid(~Drug)+ggtitle("AURKB targets")
  ggsave('aurkb_ridge.pdf',width=8,height=5)
...
new_subs=mutate(pars,Drug='Gilteritinib')|>
 ...
 ggplot(new_subs,aes(x=value,y=SiteID,fill=State,alpha=0.5))+geom_density_ridges()+scale_fill_manual(values=colors)+facet_grid(~Drug)+ggtitle("SRC targets")
  ggsave('src_ridge.pdf',height=3,width=8)
 ...
 ggplot(new_subs,aes(x=Drug,y=value,fill=State))+geom_boxplot()+scale_fill_manual(values=colors)+ggtitle("SRC targets")
  ggsave("src_targ_boxplot.pdf")
   ...
   ##lastly we have SRC targets
 aurka_subs<-subset(all_kinase_enrich,Kinase.Gene=='PRKG1')|>
   dplyr::select(SiteID='Feature',logFC,condition)|>
  distinct()|>
  #mutate(mod=tolower(substr(Substrate.Mod,1,1)))|>
  #tidyr::unite(Substrate.Gene,Substrate.Mod,col=site,sep='-')|>
...
new_subs=mutate(pars,Drug='Gilteritinib')|>
 ...
 ggplot(new_subs,aes(x=value,y=SiteID,fill=State,alpha=0.5))+geom_density_ridges()+scale_fill_manual(values=colors)+facet_grid(~Drug)+ggtitle("PRKG1 targets")
  ggsave('PRKG1_ridge.pdf')

 ggplot(new_subs,aes(x=Drug,y=value,fill=State))+geom_boxplot()+scale_fill_manual(values=colors)+ggtitle("PRKG1 targets")
  ggsave("PRKG1_targ_boxplot.pdf")
```
### Decitabine  targets
...
ggsave('dnmt1_expr.pdf',height=5,width=8)
...
kegg_hem_exp<-subset(global, Gene%in%kegg_genes$userId)|>
  subset(Exp%in%c('Gilteritinib,','Ex18'))|>
  dplyr::select(-c(State,Plex,Ligand,Exp,Treatment))|>
  left_join(meta)|>
...
ggsave('hemLineage.pdf',width=6,height=3)

##mtor

kegg_genes<-subset(all_global_kegg,description=='Cell adhesion molecules')|>
  dplyr::select(userId)|>
  tidyr::separate_rows(userId,sep=';')
...
kegg_hem_exp<-subset(global, Gene%in%kegg_genes$userId)|>
  ...
  left_join(meta)|>
  subset(Drug%in%c('none','Gilteritinib+Venetoclax'))
ggplot(kegg_hem_exp,aes(x=Drug,y=value,fill=State))+geom_boxplot()+scale_fill_manual(values=colors)+ggtitle("kegg:cell adhesion")

ggplot(kegg_hem_exp,aes(x=value,y=Gene,fill=State,alpha=0.5))+geom_density_ridges()+scale_fill_manual(values=colors)+ggtitle("kegg:cell adhesion")

ggsave('kegg_cellAdhesion.pdf',width=6,height=5)
...
p<-ggplot(tab,aes(x=`Cell type`,fill=MOLM14,y=value,color=MOLM14))+
  geom_boxplot()+
...
ggsave('fig3_deconv.pdf',p,width=5,height=3)
