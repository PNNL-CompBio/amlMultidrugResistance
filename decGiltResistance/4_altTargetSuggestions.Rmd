...
kins<-readr::read_csv(synGet('syn62013171')$path)
...
lapply(unique(top_paths$pathway),function(x){
  top_paths|>
    ...
    ggtitle(paste0('Top 20 ',x,' terms'))
  
  ggsave(filename = paste0('top20',x,'terms.pdf'),width=14)
})
...
top_kins<-kins|>arrange(BH_pvalue)|>subset(BH_pvalue<0.05)|>group_by(condition,direction)|>group_modify(~getTopN(.x))
  ...
  top_kins|>
    ...
    theme_minimal()+
    scale_color_manual(values=colc)+
    ggtitle(paste0('Top 20 Kinases'))
  
  ggsave(filename = paste0('top20Kinases.pdf'),width=8)
...
#for each condition, predict MOA and get significant values, plot in bar graph
