---
title: "patient outcomes cohort Data collection"
author: "Sara Gosline"
date: "2025-06-25"
...
##all the data in the pilot study, including beatAML samples (bridge samples)
full_meta_pilot <- rbind(comb,ba) |>
  mutate(study = 'Pilot')|>
      mutate(Proteomics=TRUE,Phospho=TRUE,Mutation=TRUE,RNASeq=TRUE,Lipidomics=TRUE,Acetylomics=TRUE,Metabolomics=TRUE)|>
  unique()

full_meta_pilot |> ggplot(aes(x=source,fill=Race)) + facet_grid(~Sex) + geom_bar(position='dodge') + scale_fill_manual(values=rcolors)
...
plotlist <- lapply(c('Proteomics','Phospho',"Mutation","RNASeq",'Lipidomics','Acetylomics','Metabolomics'),function(x)
  ba_meta |> 
    subset(get(x)) |> 
    ggplot(aes(x=source,fill=Race)) +facet_grid(~Sex) +  geom_bar(position='dodge') + scale_y_log10() + ggtitle(paste('Samples with',x,'data'))+scale_fill_manual(values=rcolors)
)
...
combined_study_meta <- rbind(select(ba_meta,-c(SampleID,`FLT3-ITDcalls`)),full_meta_pilot)
plotlist <- lapply(c('Proteomics','Phospho',"RNASeq",'Lipidomics','Acetylomics','Metabolomics'),function(x)
  combined_study_meta |> 
    subset(get(x)) |> 
    subset(!is.na(Sex))|>
    subset(!is.na(Race))|>
    ggplot(aes(x=study,fill=Race)) +facet_grid(~Sex) +  geom_bar(position='dodge') +  ggtitle(paste('Samples with',x,'data'))+scale_fill_manual(values=rcolors))
cowplot::plot_grid(plotlist=plotlist,nrow=2)
ggsave('allSampleData.png',height=6,width=12)
...
cowplot::plot_grid(plotlist=plotlist,nrow=2)
ggsave('bridgingSamples',height=6,width=16)
...
full_meta <- ba_meta|>
  dplyr::select(-c(`FLT3-ITDcalls`,SampleID))|>
