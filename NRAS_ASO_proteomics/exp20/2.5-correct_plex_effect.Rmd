...
library(synapser)
synLogin()
global_crosstab <- read.table(synGet("syn70198035")$path, sep = "\t")
phospho_crosstab <- read.table(synGet("syn52623908")$path, sep = "\t")

#meta <- read.table("data/samples.txt", sep = "\t", header = T) %>% #this doesn't exist
meta <- readxl::read_xlsx(synGet('syn52653527')$path)  |>
  select(Sample, MeasurementName, PlexID) %>%
  mutate(Time = sub("^([0-9]+ HR): MOLM14 Late Gilteritinib Resistant Flask .*$", "\\1", Sample),
         ...
         Time = case_when(grepl("baseline", Volume) ~ Volume,
                          TRUE ~ Time),
         Group = sub("^[0-9]+ HR\\: [0-9]+L \\+ 5 µM (.*)", "\\1", Volume),
...
meta<- tibble::column_to_rownames(meta,'MeasurementName')
...
saveRDS(m_global_c,file='exp_20_global.rds')
saveRDS(m_phospho_c,file='exp_20_phospho.rds')
