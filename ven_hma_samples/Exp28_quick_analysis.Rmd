...
##SG: suggest these outliers
outliers <- c('24-00144', '24-00675', '22-00184')

# meta = openxlsx::read.xlsx("metadata_Exp28.xlsx")
meta = openxlsx::read.xlsx(syn$get("syn69058992")$path)
meta$survival_days = meta[[13]]
p_data = meta %>% subset(!labId %in% outliers) |>
  mutate(sample_name = datasetNameGlobal,
...
pats <- intersect(colnames(mat), rownames(p_data))
p_data = p_data[pats, ]

m_exp28 = MSnSet(exprs = mat[,pats], pData = p_data)
