...
fulldat$`cell line`<-factor(fulldat$`cell line`,levels=c('Gilteritinib','Decitabine',"Gilteritinib+Venetoclax","Gilteritinib+Decitabine","Decitabine+Venetoclax","Gilteritinib+Decitabine+Venetoclax"))
...
subset(fulldat,`Signature type`=='Ratio')|>
  ggplot(aes(y=Signature,x=`cell line`,size=abs(difference),col=concordant,shape=significant))+geom_point()+scale_x_discrete(guide = guide_axis(angle = 90))+ggtitle("Ratio signal correlation with Cell line changes")+scale_color_manual(values=tfcolors)
