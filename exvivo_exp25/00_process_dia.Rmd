...
syn<-synapseLogin()
crosstab_prot <- read.table(syn$get('syn64604961')$path,sep='\t')#"crosstab_PTRC_wrkpckg5908_DIA_nolib.txt", sep = "\t")
# syn <- synapseLogin()
...
meta<-meta|>mutate(Drug=sapply(Treatment,function(x) druglist[[x]]))

meta<-meta|>subset(Treatment%in%c("UT","V","DV","D"))

rownames(meta) <- meta$sample
m <- MSnSet(exprs = crosstab[,meta$sample], pData = meta)
# pData(m)
# exprs(m)
...
plot_pca(m, phenotype = "patient")+scale_fill_manual(values=pcolors)+scale_color_manual(values=pcolors)

ggsave('patientPCA_uc.pdf')
plot_pca(m, phenotype = "Drug")+scale_fill_manual(values=dcolors)+scale_color_manual(values=dcolors)
ggsave('drugPCA_uc.pdf')

plot_pca(m, phenotype = "Drug",components=2:3)+scale_fill_manual(values=dcolors)+scale_color_manual(values=dcolors)
write.table(exprs(m), "ptrc_4patient_cellines_dia.txt", sep = "\t", quote = F)

saveRDS(m, "msnset_4patients_uncorrected.RDS")
synapseStore("msnset_4patients_uncorrected.RDS",'syn64134071')
...
synapseStore("msnset_4patients_corrected.RDS",'syn64961545')
plot_pca(m_corrected, "Drug")+scale_fill_manual(values=dcolors)+scale_color_manual(values=dcolors)
ggsave('drugPCA_corrected.pdf')
plot_pca(m_corrected, "patient")+scale_fill_manual(values=pcolors)+scale_color_manual(values=pcolors)
ggsave("patientPCA_corrected.pdf")

write.table(exprs(m_corrected), "4patients_corrected_crosstab.tsv", sep = "\t", quote = F)
synapseStore("4patients_corrected_crosstab.tsv",'syn64961545')

nmmat<-exprs(m_corrected)
nmmat[which(is.na(nmmat),arr.ind=T)]<-0.0
...
ggplot(pu,aes(x=Dim1,y=Dim2,col=Drug))+geom_point()
```
