...
I'm not sure what the best way is to tweak the signatures, but here are a series of tests we can perform.
...
pdf('sigPlots.pdf',width=11)
finalsigs<-do.call(rbind,lapply(treatment_contrasts,function(y){
    markers=getSigsFromData(condition=y,doPlot=TRUE)
...
res<-lapply(unique(finalsigs$treatment),function(treat){
  ...
  pheatmap::pheatmap(expr,
                     ...
                     annotation_colors = list(condition=sigcolors,Treatment=dcolors,patient=pcolors),cellwidth=10,
...
res<-do.call(rbind,lapply(treats,function(x){
...
res<-res|>tidyr::separate(sample,into=c('patient','treatment'),sep='_')
...
ggsave('signatureScoreOfSamples.pdf',height=4)
...
eres<-do.call(rbind,lapply(names(all_t2gs),function(x){
  ...
  do.call(rbind,lapply(treatment_contrasts,function(y){
    markers=getSigsFromData(condition=y)
...
ggsave('enrichedTermsQval25AtLeast2Proteins.pdf')

readr::write_csv(eres,file='functionalEnrichmentResults.csv')
```
...
paths<-eres|>subset(qvalue<0.25)|>subset(Count>1)|>
  subset(path%in%c('Reactome','GOBP'))|>
  group_by(ID)|>summarize(nsigs=n_distinct(signature))|>subset(nsigs>1)

eres$signature=factor(eres$signature,levels=c('Gilteritinib','Venetoclax','Decitabine','Decitabine+Venetoclax','Gilteritinib+Venetoclax','Gilteritinib+Decitabine','Gilteritinib+Decitabine+Venetoclax'))

##now create a bubble plot of the pathways by enrichment? 
eres|>subset(sign=='resistant')|>
  subset(ID%in%paths$ID)|>ggplot(aes(x=signature,y=ID,size=Count,col=qvalue))+geom_point()+ggtitle("Resistant signature enrichment - GO BP")+theme(axis.text.x=element_text(angle=90, hjust=1))
ggsave('resEnrichGOBPReactome.pdf',height=12)

eres|>subset(sign=='sensitive')|>
  subset(ID%in%paths$ID)|>ggplot(aes(x=signature,y=ID,size=Count,col=qvalue))+geom_point()+ggtitle("Sensitive signature enrichment - GO BP")+theme(axis.text.x=element_text(angle=90, hjust=1))
ggsave('sensEnrichGOBPReactome.pdf',height=12)
