# figure 3 for NRAS manuscript: cell line KO (exp 20)
...
dir.create("Figures")
setwd("Figures")
dir.create("suppFigExp22")
setwd("suppFigExp22")

#### RAS/MAP/ERK diffexp dot plots ####
# synIDs.mix <- list("Global" = list("NRAS KO" = "syn64735095"),
#                    "Phospho" = list("NRAS KO" = "syn64735201"))
synIDs.mix <- list("Global" = list("NRAS KO" = "syn66727085"),
                   "Phospho" = list("NRAS KO" = "syn66727197"))
...
for (t in names(all.synIDs)) {
  ...
  for (omics in names(synIDs)) {
    ...
    ggplot2::ggsave(paste0(t, "_", omics,"_RAS_diffexp_dotPlot.pdf"), 
                    width=ifelse(omics=="Global",2.5,4), 
                    height=ifelse(length(geneOrder)/4 > 49, 49, length(geneOrder)/4))
    if(omics=="Global") {
      ...
      ggplot2::ggsave(paste0(t, "_", omics,"_RAS_diffexp_dotPlot_width3.5.pdf"), 
                      width=3.5, 
                      height=ifelse(length(geneOrder)/4 > 49, 49, 0.5+length(geneOrder)/4))
    }
  } 
}
...
# synIDs.mix <- list("Global" = list("NRAS KO" = "syn64735103"),
#                    "Phospho" = list("NRAS KO" = "syn64735208"))
library(PCSF)
data("STRINGv12")
...
synIDs.mix <- list("Global" = list("NRAS KO" = "syn66727096"),
                   "Phospho" = list("NRAS KO" = "syn66727205"))
all.synIDs <- list("Mix" = synIDs.mix)
network <- TRUE
all.ea <- list()
for (t in names(all.synIDs)) {
    ...
    if (nrow(sig.ea) > 0) {
      ...
      ggplot2::ggplot(ea[ea$Feature_set2 %in% geneOrder,], 
        ...
        geom_point(data = subset(ea[ea$Feature_set2 %in% geneOrder,], sig), 
                   col = "black", stroke = 1.5, shape = 21) +
        labs(size="-Log(FDR)") + theme(axis.title=element_blank(), legend.position="left",
                                       axis.text.x=element_text(angle=45, vjust=1, hjust=1))
      ...
      if (network & omics=="Phospho") {
        ...
        if (any(grepl("MAP", ksea.vert$Gene) | grepl("RAF", ksea.vert$Gene))) {
        ...
        if (nrow(ksea.edges) > 0) {
          ...
          RCy3::createNetworkFromIgraph(temp.network, title="exp22_sig_top25_KSEA_STRINGv12")
        }
        ...
        ggplot2::ggplot(dot.df, 
          ...
          labs(size="-Log(FDR)") + theme(axis.title=element_blank(), legend.position="left",
                                         axis.text.x=element_text(angle=45, vjust=1, hjust=1),
                                         axis.text.y=element_text(color=dot.df$color))
        ...
        ggplot2::ggplot(dot.df, 
          ...
          geom_point(data = subset(dot.df, sig), 
                     col = "black", stroke = 1.5, shape = 21) +
          labs(size="-Log(FDR)") + 
          theme(axis.title=element_blank(), legend.position="left", axis.text.y=element_text(color=textColors))
        customHeight <- ifelse(length(geneOrder)/5 < 3, 3, length(geneOrder)/5)
        ...
        if (any(ksea.vert$Gene %in% mapkSignaling)) {
        ...
        if (nrow(ksea.edges) > 0) {
          ...
          RCy3::createNetworkFromIgraph(temp.network, title="exp22_sig_cellCycleMAPK_KSEA_STRINGv12") 
        }
        ...
        if (nrow(ksea.edges) > 0) {
          ...
          RCy3::createNetworkFromIgraph(temp.network, title="exp22_sig_cellCycleMAPKSphingoOXPHOS_KSEA_STRINGv12")
      ...
      if (any(ras.ea$sig)) {
        ...
        if (network) {
          ...
          if (nrow(ksea.edges) > 0) {
            ...
            RCy3::createNetworkFromIgraph(temp.network, title="exp22_sig_RAS-etc_KSEA_STRINGv12")
          }
        }
      }
...
#       }
#       
#       sig.ea2 <- sig.ea %>% slice_max(abs(NES), n=25)
#       mean.ea2 <- plyr::ddply(sig.ea2, .(Feature_set2), summarize,
#                               NES = mean(NES, na.rm=TRUE))
#       geneOrder <- na.omit(unique(mean.ea2[order(mean.ea2$NES, decreasing=TRUE),]$Feature_set2))
#       maxAbsNES <- max(abs(na.omit(ea[ea$Feature_set2 %in% geneOrder,]$NES)))
...
#       customHeight <- ifelse(omics=="Global", ifelse(length(geneOrder)/4 > 49, 49, length(geneOrder)/4),
#                              ifelse(length(geneOrder)/4 < 3, 3, length(geneOrder)/4))
#       ggplot2::ggsave(ifelse(omics=="Global", paste0(t, "_", omics,"_GSEA_KEGG_top25_dotPlot_v2.pdf"),
#                              paste0(t, "_", omics,"_KSEA_top25_dotPlot_v2.pdf")), 
#                       width=ifelse(omics=="Global",7,3), 
#                       height=customHeight) 
#     }
#     
...
ggsave("GSEA_volcanoPlots_labels.pdf", volc.plots, width = 4, height = 2.5)
ggsave("GSEA_volcanoPlots_Wider.pdf", volc.plots, width = 5, height = 2.5)

volc.plots <- NULL
#titles <- list("NRAS KO" = "0 hr", "NRAS KO + Gilt" = "6 hr")
for (contrast in unique(kegg$Contrast)) {
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_groupsOnly.pdf", volc.plots, width = 4, height = 2.5)
...
for (contrast in unique(kegg$Contrast)) {
  ...
  contr.plot <- ggplot2::ggplot(dot.df, aes(x=NES, y=minusLogP)) + geom_point(size = 4, color=dot.df$color) +
    ...
    labs(title=contrast, color="Pathway") + 
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_canonical.pdf", volc.plots, width = 4, height = 2.5)
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_canonicalSig.pdf", volc.plots, width = 4, height = 2.5)

#### DMEA ####
### drug correlations
synIDs.mix <- list("Global" = "syn66727187",
                   "Phospho" = "syn66727257")
all.synIDs <- list("Mix" = synIDs.mix)
...
for (t in names(all.synIDs)) {
  ...
  moaOrder <- c(moaOrder[moaOrder!="Other"],"Other") # nothing except Other
