...
synIDs.mix <- list("Global" = list("0" = "syn66726844",
                                   "2" = "syn66727770",
                                   "6" = "syn66729260",
                                   "24" = "syn66730823"),
                   "Phospho" = list("0" = "syn66726866",
                                    "2" = "syn66727869",
                                    "6" = "syn66729468",
                                    "24" = "syn66730918"))
...
synIDs.mix <- list("Global" = list("0" = "syn66726857",
                                   "2" = "syn66727776",
                                   "6" = "syn66729273",
                                   "24" = "syn66730829"),
                   "Phospho" = list("0" = "syn66726879",
                                    "2" = "syn66727874",
                                    "6" = "syn66729472",
                                    "24" = "syn66730923"))
all.synIDs <- list("Mix" = synIDs.mix)
  ...
  for (omics in names(synIDs)) {
    ...
    if (nrow(sig.ea) > 0) {
      ...
      if (network & omics=="Phospho") {
        ...
        RCy3::createNetworkFromIgraph(temp.network, title="exp20_sig_top25_KSEA_STRINGv12")
        ...
        keptCellCycle <- cellCycle[cellCycle %in% geneOrder] # 10
        keptMapk <- mapkSignaling[mapkSignaling %in% geneOrder] # 38
        keptBoth <- keptCellCycle[keptCellCycle %in% keptMapk] # 0?
        geneOrder <- c(sort(keptCellCycle), sort(keptMapk)) # looks like some are both because order is mixed
        "MAP2K2" %in% mapkSignaling # TRUE even though color is blue (cell cycle) - color is actually purple (MAPK) in dot.df?
        # need to order data frame by gene order so colors match
        rownames(dot.df) <- dot.df$Feature_set # duplicates because of contrasts
        ...
        RCy3::createNetworkFromIgraph(temp.network, title="exp20_sig_cellCycleMAPK_KSEA_STRINGv12")
        ...
        ksea.vert$Group <- "Cell Cycle"
        ksea.vert[ksea.vert$Gene %in% mapkSignaling,]$Group <- "MAPK"
        ksea.vert[ksea.vert$Gene %in% keptPi3kAkt,]$Group <- "PI3K/AKT"
        ksea.edges <- STRINGv12[STRINGv12$from %in% ksea.vert$Gene &
                                  STRINGv12$to %in% ksea.vert$Gene,]
        ksea.edges <- ksea.edges[ksea.edges$from != ksea.edges$to,]
        ksea.edges$combo <- NA
        ...
        RCy3::createNetworkFromIgraph(temp.network, title="exp20_sig_cellCycleMAPKAKT_KSEA_STRINGv12")
        ...
        RCy3::createNetworkFromIgraph(temp.network, title="exp20_sig_cellCycleMAPKSphingoOXPHOS_KSEA_STRINGv12")
    ...
    if (omics == "Phospho") {
      ...
      if (any(ras.ea$sig)) {
        ...
        if (network) {
          ...
          RCy3::createNetworkFromIgraph(temp.network, title="exp20_sig_RAS-etc_KSEA_STRINGv12")
        }
      }
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_groupsOnly.pdf", volc.plots, width = 14, height = 2.5)
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_canonical.pdf", volc.plots, width = 14, height = 2.5)
...
for (contrast in unique(kegg$Contrast)) {
  ...
  #dot.df$Label <- factor(dot.df$Label, levels=c("Cell Cycle","MAPK Signaling", "Oxidative Phosphorylation", "Sphingolipid Metabolism", NA))
  #dot.df <- dot.df[order(dot.df$Group),]
  contr.plot <- ggplot2::ggplot(dot.df, aes(x=NES, y=minusLogP)) + geom_point(size = 4, color=dot.df$color) +
    theme_classic(base_size=12) + labs(y="-Log(P-value)") +
    #geom_point(data = subset(dot.df, sig), col = "black", stroke = 1.5, shape = 21, size = 4) +
    scale_x_continuous(limits=c(-maxAbsNES, maxAbsNES)) + 
...
volc.plots <- volc.plots + plot_layout(nrow=1, guides="collect")
ggsave("GSEA_volcanoPlots_canonicalSig.pdf", volc.plots, width = 14, height = 2.5)

#### DMEA plots ####
### drug correlations
synIDs.mix <- list("Global" = list("0" = "syn66726847",
                                   "2" = "syn66727860",
                                   "6" = "syn66729451",
                                   "24" = "syn66730910"),
                   "Phospho" = list("0" = "syn66726869",
                                    "2" = "syn66727912",
                                    #"6" = "",
                                    "24" = "syn66730956"))
synIDs.mix <- list("Global" = "syn66726847", # only want to look at "0" aka NRAS ASO vs. CTRL ASO
                   "Phospho" = "syn66726869")
all.synIDs <- list("Mix" = synIDs.mix)
...
BCL2i <- unique(drug.info[grepl("BCL", drug.info$moa),]$Drug) # 3
FLT3i <- unique(drug.info[grepl("FLT", drug.info$moa),]$Drug) # 14
mapk.df <- as.data.frame(mapkSignaling)
colnames(mapk.df) <- "Gene"
mapk.df$Family <- sub("\\d.*", "", mapk.df$Gene)

# manually fix those ending in A, B, C
mapk.df[grepl("CACN", mapk.df$Family),]$Family <- "CACN"
mapk.df[grepl("FLN", mapk.df$Family),]$Family <- "FLN"
mapk.df[grepl("GN", mapk.df$Family),]$Family <- "GN"
mapk.df[grepl("HSP", mapk.df$Family),]$Family <- "HSP"
mapk.df[grepl("CACN", mapk.df$Family),]$Family <- "CACN"
mapk.df[grepl("IKBK", mapk.df$Family),]$Family <- "IKBK"
mapk.df[grepl("JUN", mapk.df$Family),]$Family <- "JUN"
mapk.df[grepl("PDGF", mapk.df$Family),]$Family <- "PDGF"
mapk.df[grepl("RAF", mapk.df$Family),]$Family <- "RAF"
# MAPKi <- unique(drug.info[grepl("MAPK",drug.info$moa) | grepl("MEK",drug.info$moa)|
#                             grepl("EGFR",drug.info$moa) | grepl("AKT",drug.info$moa) |
#                             grepl("PI3K",drug.info$moa) | grepl("TGFB",drug.info$moa),]$Drug) # 7
#PI3Ki <- unique(drug.info[grepl("PI3K",drug.info$moa),]$Drug) # 6
MAPKi <- c()
for (i in na.omit(unique(mapk.df$Family))) {
  if (any(grepl(i, drug.info$moa, ignore.case=TRUE))) {
    MAPKi <- unique(c(MAPKi, drug.info[grepl(i, drug.info$moa, ignore.case=TRUE),]$Drug))
  }
} # 63 drugs
mapk.drug.info <- drug.info[drug.info$Drug %in% MAPKi,]

p38MAPKi <- unique(drug.info[grepl("MAPK", drug.info$moa),]$Drug) # 7
MEKi <- unique(drug.info[grepl("MEK", drug.info$moa),]$Drug) # 5
...
for (t in names(all.synIDs)) {
  ...
  dot.df <- na.omit(merge(time.drug.corr, drug.info, by="Drug", all.x=TRUE))
  # time.drug.corr$Mechanism <- "Other"
  # time.drug.corr[time.drug.corr$Drug %in% BCL2i,]$Mechanism <- "BCL2 inhibitor"
  # time.drug.corr[time.drug.corr$Drug %in% FLT3i,]$Mechanism <- "FLT3 inhibitor"
  # time.drug.corr[time.drug.corr$Drug %in% p38MAPKi,]$Mechanism <- "p38 MAPK inhibitor"
  # time.drug.corr[time.drug.corr$Drug %in% MEKi,]$Mechanism <- "MEK inhibitor"
  # time.drug.corr$Target <- "Other"
  # time.drug.corr[time.drug.corr$Drug %in% BCL2i,]$Target <- "BCL2"
  # time.drug.corr[time.drug.corr$Drug %in% FLT3i,]$Target <- "FLT3"
  # time.drug.corr[time.drug.corr$Drug %in% p38MAPKi,]$Target <- "p38 MAPK"
  # time.drug.corr[time.drug.corr$Drug %in% MEKi,]$Target <- "MEK"
  # time.drug.corr[time.drug.corr$Drug %in% EGFRi,]$Target <- "EGFR"
  # time.drug.corr[time.drug.corr$Drug %in% EGFRi,]$Target <- "EGFR"
  
  sig.drug.corr <- dot.df[dot.df$Pearson.q < 0.05,]
  ...
  dot.df <- dot.df[dot.df$Drug %in% geneOrder,]
  ...
  dot.df$width <- 0
  dot.df[dot.df$Pearson.q <= 0.05,]$width <- 1
  ...
  ggplot2::ggplot(dot.df, aes(x=Pearson.est, y=Drug, fill=Target2)) + 
    ...
    scale_y_discrete(limits=geneOrder) + scale_x_continuous(limits=c(-1,1)) +
...
# 
# ### DMEA
# synIDs.mix <- list("Global" = list("0" = "syn66726846",
#                                    "2" = "syn66727857",
#                                    "6" = "syn66729445",
#                                    "24" = "syn66730907"),
#                    "Phospho" = list("0" = "syn66726868",
#                                     "2" = "syn66727909",
#                                     #"6" = "",
#                                     "24" = "syn66730951"))
# synIDs.mix <- list("Global" = "syn66726846", # only want to look at "0" aka NRAS ASO vs. CTRL ASO
#                    "Phospho" = "syn66726868")
# saveRDS(all.dmea, "dmea.rds")
# all.dmea <- data.table::rbindlist(all.dmea$Mix, use.names = TRUE, idcol="Omics")
# write.csv(all.dmea, "SupplementaryTable_DMEA.csv", row.names=FALSE)
# all.dmea <- readRDS("dmea.rds")
